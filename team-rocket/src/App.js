...
function App() {
  ...
  const calculateHP = (attacker, defender) => {
    ...
    return (defender.hp -
      (((BASE_DAMAGE * attacker.attack / defender.defense) + MIN_DAMAGE) *
      random / DAMAGE_NORMALIZOR));
  };
    ...
    fetch(enemyPokemon.defeat)
      ...
      .then((secondData) => {
        const pokemonProps = {
          ...
          urlBack: secondData.sprites.versions['generation-v']['black-white'].animated.back_default,
        };
      ...
      fetch(pokeURL)
        ...
        .then((secondData) => {
          const pokemonProps = {
            ...
            secondData.sprites.versions['generation-v']['black-white'].animated.front_default,
            urlBack:
            secondData.sprites.versions['generation-v']['black-white'].animated.back_default,
            hit: false,
          };
          setPokeData((prevData) => [...prevData, pokemonProps]);
        });
  ...
  return (
    <div className="App" >
      {!selectedLocation && (
        data && data.results &&
        <Locations locationList={data.results} onClick={handleLocationClick}/>
      )}

      {selectedLocation && !selectedArea && (
        data.areas &&
        <Areas areaList={data.areas} onClick={handleAreaClick} onBackClick={handleBackClick}/>
      )}
      ...
      {selectedLocation && selectedArea && data.pokemon_encounters && (
        <div>
          ...
          <EnemyPokemon
            onFind={handleFindPokemon}
            battleEnemy={enemyPokemon}
            enemyList={data.pokemon_encounters}
          />
          <MyPokemons
            pokemons={pokeData}
